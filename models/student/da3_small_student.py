...
class DA3SmallStudent(nn.Module):
    ...
    def _depth_main_chunk(
        ...
        if tokens[0].shape[1] != patch_h * patch_w:
            raise ValueError("Depth features do not match the runtime patch grid")
        resized = []
    ...
    def forward(
        ...
        if images.ndim != 5 or images.shape[0] < 1 or images.shape[2] != 3:
        ...
        elif frames != 32 or (height, width) != (224, 280):
            raise ValueError("DA3 inference requires [B,32,3,224,280]")
