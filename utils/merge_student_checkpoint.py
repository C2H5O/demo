"""Merge a DA3-Small training checkpoint into a standalone inference model.

Compatibility command, run from the project root:

    python outputs/merge_baseline_j.py

The source DA3-Small safetensors and training checkpoint are read-only.  The
only persistent output is ``ours.pt`` next to the selected training checkpoint.
"""
...
def _resolved_student_config(
    ...
    if not isinstance(checkpoint_student, Mapping):
        raise ValueError("Student checkpoint config.student is missing")
...
def merge_student_checkpoint(
    checkpoint_path: Path,
    project_config: Mapping[str, Any],
    output_path: Path | None = None,
) -> Path:
    """Merge one training checkpoint and return its atomic ``ours.pt`` path."""
    ...
    if output_path == checkpoint_path.resolve():
        raise RuntimeError("Output path must not overwrite the training checkpoint")

    checkpoint_sha_before = _sha256(checkpoint_path)
    checkpoint = _load_torch_checkpoint(checkpoint_path)
    if not isinstance(checkpoint, Mapping):
        raise ValueError("Student checkpoint root must be a mapping")
    ...
    base_config_sha_before = _sha256(base_config_path)
    ...
    base_config_sha_after = _sha256(base_config_path)
    ...
    print("Base config unchanged: {}".format(base_config_sha_before == base_config_sha_after))
    print("Training checkpoint unchanged: {}".format(checkpoint_unchanged))
    print("ours.pt file size: {}".format(_format_bytes(output_path.stat().st_size)))
    if (
        not base_unchanged
        or base_config_sha_before != base_config_sha_after
...
def ensure_merged_student_checkpoint(
    checkpoint_path: Path,
    project_config: Mapping[str, Any],
) -> Path:
    """Return a strict inference checkpoint, creating fresh ``ours.pt`` if needed."""
    checkpoint_path = Path(checkpoint_path).expanduser().resolve()
    if not checkpoint_path.is_file():
        raise FileNotFoundError("Student checkpoint not found: {}".format(checkpoint_path))
    checkpoint = _load_torch_checkpoint(checkpoint_path)
    ...
    if isinstance(metadata, Mapping) and metadata.get("format") == MERGED_STUDENT_CHECKPOINT_FORMAT:
        require_merged_student_checkpoint(dict(checkpoint))
        return checkpoint_path

    source_sha = _sha256(checkpoint_path)
    _, base_path, base_config_path = _resolved_student_config(checkpoint, project_config)
    base_sha = _sha256(base_path)
    base_config_sha = _sha256(base_config_path)
    output_path = checkpoint_path.with_name("ours.pt")
    if output_path.is_file():
        try:
            cached = _load_torch_checkpoint(output_path)
            if not isinstance(cached, dict):
                raise ValueError("Merged checkpoint root must be a mapping")
            require_merged_student_checkpoint(cached)
            cached_metadata = cached.get("merge_metadata", {})
            if (
                cached_metadata.get("source_checkpoint_sha256") == source_sha
                and cached_metadata.get("base_checkpoint_sha256") == base_sha
                and cached_metadata.get("base_config_sha256") == base_config_sha
            ):
                print("Reusing current merged student checkpoint: {}".format(output_path))
                return output_path
            print("Regenerating stale merged student checkpoint: {}".format(output_path))
        except (OSError, RuntimeError, TypeError, ValueError, KeyError) as error:
            print("Regenerating invalid merged student checkpoint {}: {}".format(output_path, error))
    return merge_student_checkpoint(checkpoint_path, project_config, output_path)


def main() -> None:
    checkpoint_path = _find_baseline_j_checkpoint()
    output_path = OUTPUT_PATH or checkpoint_path.with_name("ours.pt")
    merge_student_checkpoint(checkpoint_path, load_config(J_CONFIG_PATH), output_path)
