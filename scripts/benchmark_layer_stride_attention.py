"""One-window 224x280 CUDA benchmark: dense DA3-Small versus Baseline-H A."""
...
def main() -> None:
    ...
    if (config["inference"]["image_height"], config["inference"]["image_width"]) != (224, 280):
        raise ValueError("Baseline-H 224 benchmark requires a 224x280 model input")
    ...
    images = torch.rand(1, WINDOW, 3, 224, 280, device=device)
    metadata = _metadata()
    ...
    with DA3KVAttention(model, kv_config, WINDOW) as correctness_adapter:
        correctness_adapter.begin_window(metadata, images)
        with torch.autocast(device_type="cuda", enabled=True):
            correctness_output = model(images, include_global_points=False)
        correctness_adapter.finish_window()
    allclose = {}
    for key in ("depth", "intrinsics", "extrinsics"):
        if key not in profile_output or key not in correctness_output:
            raise RuntimeError(f"DA3 output is missing required correctness tensor: {key}")
        torch.testing.assert_close(profile_output[key], correctness_output[key])
        allclose[key] = True
    del profile_output, correctness_output
    ...
    print(json.dumps({
        ...
        "profiled_vs_unprofiled_sparse_a_allclose": allclose,
