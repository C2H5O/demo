...
repair_numpy() {
  if [[ -n "${CONDA_PREFIX:-}" ]] && command -v conda >/dev/null 2>&1; then
    # A pip NumPy layered over a different Conda numpy-base produces two
    # incompatible ndarray C types. Repair both packages in the active prefix.
    conda install --yes --force-reinstall --prefix "${CONDA_PREFIX}" \
