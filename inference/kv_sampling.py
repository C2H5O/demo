...
class KVSamplingConfig:
    enabled: bool = False
    method: str = "vda_role"
    # Retain the explicit QG policy for standalone callers; H selects the
    # role/layer spatial policy in its own YAML.
    ...
    def frame_budget(self, window_length: int, *, first_window: bool = False) -> int:
        ...
        if self.method == "query_group":
            if (self.query_group_size <= 0 or self.kv_frames <= 0
                or self.kv_frames > window_length
        ...
        if self.method == "layer_stride_kv":
            ...
            if any(forbidden.values()):
                raise ValueError("Layer-stride KV must not configure highlight or frame selection")
            return window_length
...
def resolve_spatial_sampling_options(options: Mapping | None = None) -> dict:
    ...
    values = dict(options)
    if not {"enabled", "early_layers", "late_layers"} <= set(values) or values["enabled"] is not True:
        raise ValueError("spatial_sampling must enable early_layers and late_layers")
    if set(values) - {"enabled", "early_layers", "late_layers", "pattern", "staggered"}:
        raise ValueError("Unknown spatial_sampling fields")
    pattern = values.get("pattern", "fixed")
    if pattern not in {"fixed", "staggered"}:
        raise ValueError("Unknown spatial_sampling.pattern")
    if pattern == "staggered":
        staggered = values.get("staggered")
        if (not isinstance(staggered, Mapping) or set(staggered) !=
            {"enabled", "cycle", "layer_phase_offsets"}
            or staggered["enabled"] is not True or staggered["cycle"] != 4
            or type(staggered["cycle"]) is not int
            or staggered["layer_phase_offsets"] != {5: 0, 7: 2}):
            raise ValueError("Staggered spatial sampling requires cycle 4 and block offsets 5:0, 7:2")
    elif "staggered" in values:
        raise ValueError("staggered options require pattern=staggered")
...
def build_spatial_patch_indices(grid_height: int, grid_width: int, stride: int,
                                row_offset: int = 0, col_offset: int = 0) -> tuple[int, ...]:
    """Row-major original-grid indices for one spatial lattice."""
    ...
    if any(type(offset) is not int or not 0 <= offset < stride for offset in (row_offset, col_offset)):
        raise ValueError("Spatial offsets must be integers within the stride")
...
def provider_spatial_plan(metadata: WindowFrameMetadata, selected_slots: Sequence[int],
                          layer_index: int, options: Mapping) -> list[dict]:
    """Assign phases in original VDA selected-provider order, before DA3 permutation."""
    selected = list(selected_slots)
    if not selected or selected != sorted(set(selected)):
        raise ValueError("Selected provider slots must be unique and in VDA temporal order")
    ...
    values = resolve_spatial_sampling_options(options)
    strides = spatial_strides_for_layer(layer_index, values)
    plan = []
    for provider_rank, slot in enumerate(selected):
        role = metadata.frame_roles[slot]
        stride = strides[role]
        phase = None
        row_offset = col_offset = 0
        if values["pattern"] == "staggered" and stride == 2:
            rotation = values["staggered"]["layer_phase_offsets"].get(layer_index)
            if rotation is None:
                raise ValueError("Sparse staggered layer needs a phase rotation")
            phase = (provider_rank + rotation) % values["staggered"]["cycle"]
            row_offset, col_offset = _PHASE_OFFSETS[phase]
        plan.append({"layer": layer_index, "provider_slot": slot,
                     "provider_rank": provider_rank, "role": role,
...
def build_role_layer_patch_indices(metadata: WindowFrameMetadata, selected_slots: Sequence[int],
    ...
    plan = provider_spatial_plan(metadata, selected_slots, layer_index, options)
    cache = {}
    patches = {}
    for item in plan:
        stride = item["stride"]
        if item["phase"] is not None and (grid_height % 2 or grid_width % 2):
            raise ValueError("Staggered stride-2 requires an even patch grid for equal KV budgets")
        lattice = (stride, item["row_offset"], item["col_offset"])
        if lattice not in cache:
            cache[lattice] = build_spatial_patch_indices(grid_height, grid_width, *lattice)
        indices = cache[lattice]
        if stride == 2 and grid_height % 2 == grid_width % 2 == 0:
            assert len(indices) == grid_height * grid_width // 4, "stride-2 KV budget changed"
        patches[item["provider_slot"]] = indices
    return patches
